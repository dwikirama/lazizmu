...
if (isset($_POST['kirim'])) {
    ...
    if (in_array($ekstensi, $valid_ekstensi)) {
        ...
        if ($ukuran < 1000000) {
            ...
            $stmt = $conn->prepare("INSERT INTO badaneksekutif (nama, jabatan, foto) VALUES (?, ?, ?)");
...
if (isset($_POST['ubah'])) {
    ...
    if (!empty($_FILES['ubah_foto']['name'])) {
        ...
        $data_lama = $conn->query("SELECT foto FROM badaneksekutif WHERE id='$id'")->fetch_assoc();
    ...
    $stmt = $conn->prepare("UPDATE badaneksekutif SET nama=?, jabatan=?, foto=? WHERE id=?");
...
$perintah = "SELECT * FROM badaneksekutif ORDER BY id ASC LIMIT $batasAwalData, $jumlahDataPerHalaman";
...
<html lang="en">
  <head>
  	<title>Halaman Lihat Data Badan Eksekutif</title>
    <meta charset="utf-8">
  ...
  <body>
    ...
    <div class="tombol-kontainer">
        ...
        <h2 style="text-align: center">Data Struktur Manajemen (Badan Eksekutif) </h2>
                ...
                <tr class="highlight">
                    ...
                    <td>
                        <div class="aksi-container">
                            ...
                            <a href="edit_data.php?jenis=badaneksekutif&id=<?php echo $data['id']; ?>" class="icon-btn btn-edit" onmouseover="zoomIn(this)" onmouseout="zoomOut(this)">
                                <ion-icon name="create"></ion-icon>
                            </a>

                            <!-- Ubah bagian ini pada tombol "Delete" -->
                            <a href="del.php?del=<?php echo $data['id']; ?>&jenis=badaneksekutif" onclick="return confirm('Apakah Anda yakin ingin menghapus data ini?')" class="icon-btn btn-delete" onmouseover="zoomIn(this)" onmouseout="zoomOut(this)">
